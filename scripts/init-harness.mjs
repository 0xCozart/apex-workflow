...
async function buildConfig(targetRoot, args) {
  ...
  try {
    ...
    if (discovery?.verificationPreset) {
      verification.defaultPreset = discovery.verificationPreset.defaultPreset;
      verification.broadChecksRunLast = discovery.verificationPreset.broadChecksRunLast;
      verification.expensiveCommandPolicy = discovery.verificationPreset.expensiveCommandPolicy;
      verification.presets = discovery.verificationPreset.presets;
    }
    const codeIntelligence = inferCodeIntelligence(targetRoot, pkg, args);
    if (discovery) {
      codeIntelligence.confidenceByTarget = {
        symbol: codeIntelligence.provider === "focused-search" ? "advisory" : "high",
        type: codeIntelligence.provider === "focused-search" ? "advisory" : "high",
        module: codeIntelligence.provider === "focused-search" ? "advisory" : "medium",
        file_path: "measure",
        new_file: "measure",
        manifest_file: "measure",
        script: "measure",
        workflow: "measure",
        documentation: "advisory",
      };
      codeIntelligence.requireFor = [];
      codeIntelligence.advisoryFor = ["documentation"];
      codeIntelligence.skipFor = [];
      codeIntelligence.fallbackEvidence = "focused_source_review";
    }
    ...
    return {
      ...
      ...(discovery
        ? {
            ...
            sliceTemplates: {},
            finishPacket: template.finishPacket,
          }
        : {}),
      authority: {
        ...authority,
      ...
      setup: {
        ...
        discovery: discovery
          ? {
              ...
              operatingModelReason: discovery.operatingModel.reason,
            }
          : { enabled: false },
        apexRoot: APEX_ROOT,
        targetRoot,
...
function printInstallReport({ config, targetRoot, dryRun }) {
  ...
  if (config.setup?.discovery?.enabled) {
    console.log("- discovery:");
    console.log(`  - operating model: ${config.operatingModel?.default ?? "unknown"}`);
    console.log(
      `  - ecosystems: ${(config.setup.discovery.ecosystems ?? [])

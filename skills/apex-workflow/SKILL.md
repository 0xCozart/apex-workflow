...
Public repo hygiene: the managed `.gitignore` block is intended to be committed so local Apex files stay out of the
target repo. `apex.workflow.json`, slice manifests, detect outputs, observation logs, command logs, finish evidence, and
...
open-source artifact. If a repo needs grant, audit, or reviewer evidence to be committed, configure
`manifest.defaultDir` to a deliberate public path such as `docs/proof/apex-workflow/` or `.apex/manifests/`, then review
the contents for secrets and private execution details before committing.
